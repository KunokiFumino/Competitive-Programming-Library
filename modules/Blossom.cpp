/*
* Use class Blossom instead. And don't ever even look at this.
*/
template <typename Twe = int> class MaximumWeightedMatching {
    /*
...
private:
    ...
    static constexpr Twe Inf = Twe(1) << (sizeof(Twe) * 8 - 2);
...
public:
    ...
    static inline vector<InputEdge> getInputEdgeVector() { return vector<InputEdge>(); }
...
private:
    ...
    struct Event {
        Event() {}
        Event(Twe time, int id) : time(time), id(id) {}
    ...
    struct EdgeEvent {
        EdgeEvent() {}
        EdgeEvent(Twe time, int from, int to) : time(time), from(from), to(to) {}
...
public:
    ...
    pair<Twe, vector<int> > maximum_weighted_matching(bool init_matching = false) {
        initialize();
        set_potential();
        if (init_matching) find_maximal_matching();
        for (int u = 1; u <= N; ++u)
            if (!mate[u]) do_edmonds_search(u);
        Twe ret = compute_optimal_value();
        return make_pair(ret, mate);
    }

private:
    Twe compute_optimal_value() const {
        Twe ret = 0;
        for (int u = 1; u <= N; ++u)
            if (mate[u] > u) {
                Twe max_c = 0;
    ...
    inline Twe reduced_cost(int u, int v, const Edge& e) const {
        return Twe(potential[u]) + potential[v] - e.cost;
    }
    ...
    template <Label Lab> inline Twe fix_blossom_potential(int b) {
        // Return the amount.
        // (If v is an atom, the potential[v] will not be changed.)
        Twe d = lazy[b];
        lazy[b] = 0;
        if (Lab == kInner) {
            Twe dt = time_current_ - time_created[b];
    ...
    template <Label Lab> inline void update_heap2(int x, int y, int by, Twe t) {
        if (t >= slack[y]) return;
        slack[y] = t;
        best_from[y] = x;
        if (y == by) {
    ...
    void push_outer_and_fix_potentials(int v, Twe d) {
        label[v] = kOuter;
        if (v > N) {
    ...
    void move_to_largest_blossom(int bid) {
        ...
        Twe d = (time_current_ - time_created[bid]) + lazy[bid];
        lazy[bid] = 0;
        for (int beta = base[bid], b = beta;;) {
    ...
    bool augment(int root) {
        ...
        while (!que.empty()) {
            ...
            for (int eid = ofs[x]; eid < ofs[x + 1]; ++eid) {
                ...
                Label l = label[by];
                if (l == kOuter) {
                    Twe t = reduced_cost(x, y, e) >> 1;  // < 2 * Inf
                    if (t == time_current_) {
                        contract(x, y, eid);
                        bx = surface[x];
                    }
                    ...
                    Twe t = reduced_cost(x, y, e);  // < 3 * Inf
                    if (t >= Inf) continue;
                    if (l != kInner) {
                        if (Twe(t) + lazy[by] == time_current_) {
    ...
    bool adjust_dual_variables(int root) {
        ...
        // -- events --
        Twe time_next = min(min(time1, time2), min(time3, time4));
...
private:
    ...
    vector<Twe> time_created, lazy, slack;
    ...
    ModifiableHeap<Twe> heap4;
};

template <typename Twv = int, typename Twe = int> class Blossom {
public:
    int N;
    Graph<Twv, Twe>* g_;
    Twe mW;
    ...
    void run() {
        auto edges = MaximumWeightedMatching<Twe>::getInputEdgeVector();
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < g_->edges[i].size(); j++) {
                auto& e = g_->get_edge(i, j);
                if (e.inv != -1 && e.inv < g_->edges[i][j]) continue;
                edges.push_back({ e.a + 1,e.z + 1,e.w });
            }
        }
        auto ret = MaximumWeightedMatching<Twe>(N, edges).maximum_weighted_matching();
        mW = ret.first;
        mate.resize(N);
        for (int i = 0; i < N; i++) mate[i] = ret.second[i + 1] - 1;
    }
};